...
export default function MapPage() {
  ...
  useEffect(() => {
    ...
    try {
      map.current = L.map(element.current, { zoomControl: false }).setView([20.5937, 78.9629], 5);
      ...
      console.error('Unable to initialise map library:', error.message);
      setMapError('Unable to load map right now. Please refresh and try again.');
    }
    return () => { resizeObserver?.disconnect(); map.current?.remove(); map.current = null; };
  }, []);
  ...
  useEffect(() => {
    ...
    filtered.forEach(r => {
      ...
      const marker = L.marker(position, { icon: L.divIcon({ className: 'violation-marker', iconSize: [32, 32], iconAnchor: [16, 16], html: `<span style="background:${color}">${ICONS[r.violationType] || '•'}</span>` }) });
